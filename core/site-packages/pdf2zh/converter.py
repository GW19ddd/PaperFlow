...
# 译文文本 bbox 相对基线的纵向范围（em 比例）。pdfminer 的 LTChar 只按 fontsize 给一个方框，
# 这里用 GoNoto/tiro 这类字体的 ascender/descender 经验值，能覆盖中英文混排的实际字形高度。
TEXT_ASCENT = 0.88
TEXT_DESCENT = 0.24
...
class TranslateConverter(PDFConverterEx):
    def __init__(
        ...
        # 段落映射 sidecar 记录表：{原文页码(0-based): {"width", "height", "paragraphs"}}。
        # 排版时一个段落会被拆成多个文本绘制对象（折行 / 中英文字体切换 / 公式占位符各切一刀），
        # 外部无法用单个矩形框住整段，所以这里顺手记下"原文段落 bbox -> 该段落产出的各译文文本 bbox"。
        # 只做记录、不参与渲染，落盘侧全程 try/except 静默降级，因此不需要任何开关。
        self.para_map: Dict[int, dict] = {}
    ...
    def receive_layout(self, ltpage: LTPage):
        ...
        # sidecar 记录开关：receive_layout 也会被 Form XObject（end_figure）调用，
        # 那份结果并不写回页面，只对真正的页面(LTPage)记录，避免污染映射表
        map_page = None
        if isinstance(ltpage, LTPage):
            try:
                map_page = self.para_map.setdefault(
                    ltpage.pageid,
                    {
        ...
        for id, new in enumerate(news):
                ...
                if (                                # 输出文字缓冲区
                    ...
                    if cstk:
                        ops_vals.append({
                            ...
                            "size": size,
                            "x": tx,
                            # 段宽直接取排版累积的字符宽度(x 已从 tx 走到当前位置)，不另行估算
                            "w": x - tx,
                            "dy": 0,
                ...
                if vy_regex:  # 插入公式
                    ...
                    for vch in var[vid]:  # 排版公式字符
                        ...
                        ops_vals.append({
                            ...
                            # 公式字符宽度取该字符自身的推进量(与绘制时同尺寸同字体)
                            "w": getattr(vch, "adv", 0.0),
            ...
            # 记录该段落 -> 各译文文本对象的 bbox（渲染循环保持原样，记录独立进行）。
            # 坐标仍是 pdfminer 的(原点左下)，翻转到 fitz(原点左上)在落盘时统一做，
            # 这样页面尺寸可以取自 PyMuPDF 的 page.rect，和外部的坐标系严格一致。
            if map_page is not None:
                try:
                    trans = []
                    for vals in ops_vals:
                        if vals["type"] != OpType.TEXT or not vals.get("w"):
                            continue
                        vx = vals["x"]
                        vy = vals["dy"] + y - vals["lidx"] * size * line_height  # 基线纵坐标
                        vsize = vals["size"]
                        trans.append((vx, vy - TEXT_DESCENT * vsize, vx + vals["w"], vy + TEXT_ASCENT * vsize))
                    if trans:
                        para = pstk[id]
                        map_page["paragraphs"].append({
                            "orig": (para.x0, para.y0, para.x1, para.y1),
                            "trans": trans,
                        })
                except Exception:  # 记录失败绝不能影响翻译
                    pass
