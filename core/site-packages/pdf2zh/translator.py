...
# v2.3.23: OpenAI 兼容服务（OpenAI / 阿里百炼 DashScope / 智谱 / 硅基…）429 限流统一退避策略。
# converter 是多线程并发翻译，所有 worker 几乎同时撞限流；固定间隔重试会让它们再次同时
# 重试、继续被限流。这里用「指数退避 + 随机抖动」把重试时刻打散（约 2/4/8/16/32/60s）。
_OPENAI_429_RETRY = retry(
    retry=retry_if_exception_type(openai.RateLimitError),
    stop=stop_after_attempt(8),
    wait=wait_exponential_jitter(initial=2, max=60, jitter=5),
    before_sleep=lambda retry_state: logger.warning(
        "429 RateLimitError, retry in %.1fs (attempt %d)",
...
class GoogleTranslator(BaseTranslator):
    ...
    def __init__(self, lang_in, lang_out, model, ignore_cache=False, **kwargs):
        ...
        # v2.3.22: 主通道换 gtx 官方免费接口 — /m 手机网页版反爬最严（429 会重定向
        # /sorry 人机验证页, IP 被临时拉黑, 等待重试也没用）; gtx 限流阈值高得多。
        self.endpoint_gtx = "https://translate.googleapis.com/translate_a/single"
        self.endpoint_m = "https://translate.google.com/m"

    def _do_translate_gtx(self, text):
        ...
        data = response.json()
        # 响应形如 [[["译文","原文",null,null,10], ...], null, "en", ...]
        return "".join(seg[0] for seg in data[0] if seg and seg[0])

    def _do_translate_m(self, text):
        response = self.session.get(
            self.endpoint_m,
    ...
    def do_translate(self, text):
        ...
        try:
            result = self._do_translate_gtx(text)
        except Exception:
            # gtx 失败时回退旧 /m 通道（两者限流策略独立, 交替兜底）
            result = self._do_translate_m(text)
        return remove_control_characters(html.unescape(result or ""))
...
class BingTranslator(BaseTranslator):
    ...
    # Bing 网页版单次请求有硬长度上限（约 1000 字符，超出部分被静默丢弃 → 段落残缺），
    # 所以长段落必须拆成多块分别翻译再拼接（这也是之前译文残缺的根本原因）。但切点绝不能
    # 落在公式占位符 {v0}/{{v0}} 或富文本标记 <b0></b0> 中间，否则占位符被劈成两半，
    # 译文里公式/加粗会直接丢失。
    _MAX_CHARS = 900
    _PLACEHOLDER_RE = re.compile(r"\{\{?v\d+\}?\}|</?b\d+>")

    def _protected_ranges(self, text):
        return [m.span() for m in self._PLACEHOLDER_RE.finditer(text)]

    @staticmethod
    def _cut_allowed(pos, protected):
        # 切点 pos 表示在 pos 之前下刀；落在某个占位符内部即为非法
        return not any(s < pos < e for s, e in protected)

    def _split_by_length(self, text):
        """按长度切块返回列表；切点优先句子边界 → 空白 → 硬切，且始终避开占位符内部。"""
        if len(text) <= self._MAX_CHARS:
            return [text]
        protected = self._protected_ranges(text)
        chunks = []
        start = 0
        while len(text) - start > self._MAX_CHARS:
            window = text[start:start + self._MAX_CHARS]
            cut = None
            for m in re.finditer(r"[.!?。！？；;]\s", window):
                if self._cut_allowed(start + m.end(), protected):
                    cut = start + m.end()
            if cut is None:
                for m in re.finditer(r"\s", window):
                    if self._cut_allowed(start + m.end(), protected):
                        cut = start + m.end()
            if cut is None:  # 没有任何合法空白/句读可用 → 硬切并回退到占位符外
                cut = start + len(window)
                while cut > start and not self._cut_allowed(cut, protected):
                    cut -= 1
            if cut <= start:  # 兜底防死循环
                cut = start + len(window)
    ...
    def _translate_chunk(self, part, sid):
        url, ig, iid, key, token = sid
    ...
    def do_translate(self, text):
        if not text or not text.strip():
            return text
        parts = self._split_by_length(text)
        sid = self.find_sid()
        results = []
        for part in parts:
            try:
                results.append(self._translate_chunk(part, sid))
            except Exception as e:
                # Bing 的 token/会话失效很常见 → 重新取 sid 再试一次；仍失败则保留原文片段，
                # 宁可局部漏译也不要把整段丢掉
                logger.warning("Bing 翻译失败，重新获取会话重试: %s", e)
                sid = self.find_sid()
                try:
                    results.append(self._translate_chunk(part, sid))
                except Exception as e2:
                    logger.warning("Bing 重试仍失败，保留原文: %s", e2)
                    results.append(part)
        translated = "".join(results)
        # 占位符完整性自检：缺了就是 Bing 把公式吃掉了，写日志便于排查
        lost = [p for p in self._PLACEHOLDER_RE.findall(text)
                if p not in translated]
        if lost:
            logger.warning("Bing 译文缺失占位符 %s", lost)
        return translated
...
class OpenAITranslator(BaseTranslator):
    ...
    def _create_chat_completion(self, **kwargs):
        """所有 OpenAI 兼容请求都必须走这里。

        子类（qwen-mt / zhipu …）重写 do_translate 时若直接调 self.client，
        会绕过父类的重试装饰器 → 一次 429 限流就整篇翻译失败。这里统一收口。
        """
